...
class EvenementDetailController extends ChangeNotifier {
  ...
  Future<void> init(int eventId) async {
    ...
    error = null;
    developer.log(
      '[EvenementDetailController] 🟡 init() starting (eventId=$eventId)',
    );
    notifyListeners();
    ...
    try {
      ...
      developer.log(
        '[EvenementDetailController] ✅ Event loaded: ${event?.nom}',
      );
      await loadTentes();
      allTentes = await tenteRepo.getTentList();
      developer.log(
        '[EvenementDetailController] ✅ Loaded ${allTentes.length} tentes',
      );
      await _loadPlan(eventId);
    } catch (e, st) {
      error = e.toString();
      developer.log(
        '[EvenementDetailController] ❌ init() failed',
        error: e,
        stackTrace: st,
      );
    } finally {
  ...
  Future<void> loadTentes() async {
    ...
    try {
      ...
      allTentes = await tenteRepo.getTentList();
      availableTentes = await tenteRepo.getAvailableTent(
        event!.date,
        event!.dateFin,
      );
      developer.log(
        '[EvenementDetailController] ✅ Loaded ${availableTentes.length} available tents',
      );
    } catch (e, st) {
      developer.log(
        '[EvenementDetailController] ❌ loadTentes failed',
        error: e,
        stackTrace: st,
      );
      error = e.toString();
  ...
  Future<void> applyTenteChanges() async {
    ...
    try {
      ...
      final updatedEvent = event!.copyWith(
        associatedTents: updatedIds.toList(),
      );
      event = updatedEvent;
      ...
      developer.log(
        '[EvenementDetailController] ❌ applyTenteChanges failed',
        error: e,
        stackTrace: st,
      );
      error = e.toString();
  ...
  Future<void> _loadPlan(int eventId) async {
    developer.log(
      '[EvenementDetailController] 🔵 _loadPlan() start (eventId=$eventId, '
      'date=$this.dayOffset, meal=$selectedMeal, offset=$dayOffset), dayOffset=$dayOffset',
    );

    try {
      final start = DateTime.now();
      currentPlan = await eventRepo.getMealPlan(
        eventId,
        dayOffset,
        selectedMeal,
      );
      ...
      developer.log(
        '[EvenementDetailController] ✅ _loadPlan() success '
        '(${currentPlan?.items.length ?? 0} items, ${duration}ms)',
      );
    } catch (e, st) {
      error = e.toString();
      developer.log(
        '[EvenementDetailController] ❌ _loadPlan() failed',
        error: e,
        stackTrace: st,
      );
    }
  ...
  Future<void> setPortions(int portions) async {
    ...
    try {
      ...
      developer.log(
        '[EvenementDetailController] ❌ setPortions failed',
        error: e,
        stackTrace: st,
      );
    }
  ...
  Future<void> createRecipes(List<Recipe> recipes) async {
    ...
    developer.log(
      '[EvenementDetailController] ➕ createRecipes(${recipes.length})',
    );
    ...
    developer.log(
      '[EvenementDetailController] ✅ $successCount/${recipes.length} recettes créées avec succès',
    );
  }
  ...
  Future<void> addMenuItemToMealPlan(MenuItem item) async {
    ...
    try {
      ...
      developer.log(
        '[EvenementDetailController] ❌ addRecipeToMealPlan failed',
        error: e,
        stackTrace: st,
      );
    }
  ...
  Future<void> removeRecipeAt(int index) async {
    ...
    developer.log(
      '[EvenementDetailController] 🗑️ removeRecipeAt(index=$index)',
    );
    ...
    try {
      ...
      developer.log(
        '[EvenementDetailController] ❌ removeRecipeAt failed',
        error: e,
        stackTrace: st,
      );
      error = e.toString();
  ...
  Future<void> moveRecipe(int oldIndex, int newIndex) async {
    if (currentPlan == null) return;
    developer.log(
      '[EvenementDetailController] 🔀 moveRecipe($oldIndex → $newIndex)',
    );
    ...
    try {
      ...
      developer.log(
        '[EvenementDetailController] ❌ moveRecipe failed',
        error: e,
        stackTrace: st,
      );
    }
  ...
  Future<void> duplicateTo(List<MapEntry<DateTime, MealType>> targets) async {
    if (currentPlan == null) return;
    developer.log(
      '[EvenementDetailController] 📄 duplicateTo() → ${targets.length} targets',
    );
    try {
      final start = DateTime.now();
      await eventRepo.duplicateMealPlans(
        event!.id.toString(),
        currentPlan!,
        targets,
      );
      final duration = DateTime.now().difference(start).inMilliseconds;
      developer.log(
        '[EvenementDetailController] ✅ duplicateMenuUC done (${duration}ms)',
      );
    } catch (e, st) {
      developer.log(
        '[EvenementDetailController] ❌ duplicateMenuUC failed',
        error: e,
        stackTrace: st,
      );
    }
...
final evenementDetailProvider =
    ChangeNotifierProvider.family<EvenementDetailController, int>((
      ref,
      eventId,
    ) {
